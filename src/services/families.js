...
const getFamilyById = async ({ id, accessToken }) => {
  const getFamilyByIdUri = template
    .parse(config.uris.familiesUri.getByIdUri)
...
const getAllFamilies = async ({ accessToken }) => {
  const getAllFamiliesUri = template
    .parse(config.uris.familiesUri.getAllWOrderUri)
    .expand({ accessToken });
...
const getFamilyApgById = async ({ id, accessToken }) => {
  const getFamilyApgByIdUri = template
    .parse(config.uris.familiesApgUri.getByIdUri)
    .expand({ id, accessToken });
...
const getAllFamiliesApg = async ({ accessToken }) => {
  const getAllFamiliesApgUri = template
    .parse(config.uris.familiesApgUri.getAllWOrderUri)
    .expand({ accessToken });
...
const putFamily = async ({ data, accessToken }) => {
  const familyUri = template
    .parse(config.uris.familiesUri.baseUri)
    .expand({ accessToken });
  await axios.put(familyUri, data);
};

const putFamilyApg = async ({ data, accessToken }) => {
  const familyApgUri = template
    .parse(config.uris.familiesApgUri.baseUri)
    .expand({ accessToken });
