...
const getAll = async ({ accessToken }) => {
  const getAllUri = template
    .parse(config.uris.usersUri.getAllWOrderUri)
    .expand({ accessToken });
...
const getByIdWithRoles = async ({ id, accessToken }) => {
  const getByIdUri = template
    .parse(config.uris.usersUri.getByIdWithRolesUri)
...
const getGeneraByUserId = async ({ id, accessToken }) => {
  const getByIdUri = template
    .parse(config.uris.usersUri.getGeneraByUserId)
    .expand({ id, accessToken });
  const response = await axios.get(getByIdUri);
  return response.data;
};
...
const createUser = async ({ data, accessToken }) => {
  const usersUri = template
    .parse(config.uris.usersUri.baseUri)
    .expand({ accessToken });
...
const updateUser = async ({ id, data, accessToken }) => {
  const usersUri = template
    .parse(config.uris.usersUri.updateByIdUri)
    .expand({ id, accessToken });
